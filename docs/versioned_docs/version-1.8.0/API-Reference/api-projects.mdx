...
Use the `/projects` endpoint to create, read, update, and delete [idrflow projects](/concepts-flows#projects).

## Read projects

Get a list of idrflow projects, including project IDs, names, and descriptions.
...
```bash
curl -X GET \
  "$IDRFLOW_URL/api/v1/projects/$PROJECT_ID" \
  -H "accept: application/json" \
  -H "x-api-key: $IDRFLOW_API_KEY"
```
...
Import a project and its flows by uploading a idrflow project zip file:
