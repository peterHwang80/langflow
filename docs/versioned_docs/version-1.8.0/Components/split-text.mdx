...
The **Chunks** output returns a list of [`Data`](/data-types#json) objects containing individual text chunks.
The **DataFrame** output returns the list of chunks as a structured [`DataFrame`](/data-types#table) with additional `text` and `metadata` columns.
