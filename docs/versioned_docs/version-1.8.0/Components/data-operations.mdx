...
The **Data Operations** component performs operations on [`Data`](/data-types#json) objects, including extracting, filtering, and editing keys and values in the `Data`.
...
For more information about the webhook endpoint, see [Trigger flows with webhooks](/webhook).
    ...
    ```bash
    curl -X POST "http://$IDRFLOW_SERVER_URL/api/v1/webhook/$FLOW_ID" \
