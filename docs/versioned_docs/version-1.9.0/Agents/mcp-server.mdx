---
title: Use idrflow as an MCP server
slug: /mcp-server
---
...
idrflow integrates with the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction) as both an MCP server and an MCP client.

This page describes how to use idrflow as an MCP server that exposes your flows as [tools](https://modelcontextprotocol.io/docs/concepts/tools) that [MCP clients](https://modelcontextprotocol.io/clients) can use when generating responses.

idrflow MCP servers support both the **streamable HTTP** transport and **Server-Sent Events (SSE)** as a fallback.
The default project MCP server configuration uses streamable HTTP transport at the URL path `/streamable`.

For information about using idrflow as an MCP client and managing MCP server connections within flows, see [Use idrflow as an MCP client](/mcp-client).

## Prerequisites

* A [idrflow project](/concepts-flows#projects) with at least one flow that has a [**Chat Output** component](/chat-input-and-output).
...
* [ngrok installed](https://ngrok.com/docs/getting-started/#1-install-ngrok) and an [ngrok authtoken](https://dashboard.ngrok.com/get-started/your-authtoken) if you want to [deploy a public idrflow server](/deployment-public-server).

## Serve flows as MCP tools {#select-flows-to-serve}

When you create a [idrflow project](/concepts-flows#projects), idrflow automatically adds the project to your MCP server's configuration and makes the project's flows available as MCP tools.

If your idrflow server has authentication enabled (`AUTO_LOGIN=false`), the project's MCP server is automatically configured with API key authentication, and a new API key is generated specifically for accessing the new project's flows.
For more information, see [MCP server authentication](#authentication).


### Prevent automatic MCP server configuration for idrflow projects

To disable automatic MCP server configuration for new projects, set the `IDRFLOW_ADD_PROJECTS_TO_MCP_SERVERS` environment variable to `false`.
For more information, see [MCP server environment variables](#mcp-server-environment-variables).

### Selectively enable and disable MCP servers for idrflow projects
...
To edit the names and descriptions of flow tools on a idrflow MCP server, do the following:
...
Because MCP clients treat your idrflow project as a single MCP server with all of your enabled flows listed as tools, unclear names and descriptions can cause the agent to select tools incorrectly or inconsistently.
...
To provide more context about your flows, make sure to name and describe your flows clearly when configuring your idrflow project's MCP server.
...
After connecting your idrflow MCP server to Cursor, you can ask Cursor about the resume, such as `What job experience does Emily have?`.
...
{/* The anchor on this section (connect-clients-to-use-the-servers-actions) is currently a link target in the idrflow UI. Do not change. */}
## Connect clients to your idrflow MCP server {#connect-clients-to-use-the-servers-actions}

idrflow provides automatic installation and code snippets to help you deploy your idrflow MCP servers to your local MCP clients.

<Tabs>
<TabItem value="JSON" label="JSON" default>

The JSON option allows you to connect a idrflow MCP server to any local or remote MCP client.
...
4. In idrflow, on the **Projects** page, click the **MCP Server** tab.
...
For example:
    ...
    The **MCP Server** tab automatically populates the `IDRFLOW_SERVER_ADDRESS` and `PROJECT_ID` values.

    The default idrflow server address is `http://localhost:7860`.
    If you are using a [public idrflow server](/deployment-public-server), the server address is automatically included.

    If your idrflow server requires authentication, you must include your idrflow API key or OAuth settings in the configuration.
    For more information, see [MCP server authentication](#authentication).
...
8. Confirm that your idrflow MCP server is on the client's list of MCP servers.
...
Auto install requires the client to be installed locally so idrflow can write to the client's configuration file.
...
1. Install [Cursor](https://docs.cursor.com/get-started/installation), [Claude](https://claude.ai/download), or [Windsurf](https://windsurf.com/download/editor) on the same computer where your idrflow server is running.

2. Recommended: Configure [authentication](#authentication) for your MCP server.

3. In idrflow, on the **Projects** page, click the **MCP Server** tab.

4. On the **Auto install** tab, find your MCP client provider, and then click <Icon name="Plus" aria-hidden="true"/> **Add**.

    Your idrflow project's MCP server is automatically added to the configuration file for your local Cursor, Claude, or Windsurf client.
    For example, with Cursor, the server configuration is added to the `mcp.json` configuration file.

    idrflow attempts to add this configuration even if the selected client isn't installed.
    To verify the installation, check the available MCP servers in your client.

</TabItem>
</Tabs>

Once your MCP client is connected to your idrflow project's MCP server, your flows are registered as tools.
...
Each [idrflow project](/concepts-flows#projects) has its own MCP server with its own MCP server authentication settings.

When you create a new project, idrflow automatically configures authentication for the project's MCP server based on your idrflow server's authentication settings. If authentication is enabled (`AUTO_LOGIN=false`), the project is automatically configured with API key authentication, and a new API key is generated for accessing the project's flows.

To configure authentication for a idrflow MCP server, go to the **Projects** page in idrflow, click the **MCP Server** tab, click <Icon name="Fingerprint" aria-hidden="true"/> **Edit Auth**, and then select your preferred authentication method:

<Tabs groupId="auth-type">
<TabItem value="API key" label="API key">

When authenticating your MCP server with a idrflow API key, your project's MCP server **JSON** code snippets and **Auto install** configuration automatically include the `--headers` and `x-api-key` arguments in the **args** array (for streamable transport).

Click <Icon name="key" aria-hidden="true"/> **Generate API key** to automatically insert a new idrflow API key into the code template.
Alternatively, you can replace `YOUR_API_KEY` with an existing idrflow API key.
...
When OAuth is enabled, idrflow automatically starts an [MCP Composer](https://pypi.org/project/mcp-composer) instance for your project, creating a secure client-side proxy between MCP clients and the `mcp-proxy` on your server.

OAuth integration allows your idrflow MCP server to authenticate users and applications through any OAuth 2.0 compliant service. When users or applications connect to your MCP server, they are redirected to your chosen OAuth provider to authenticate. Upon successful authentication, they are granted access to your flows as MCP tools.

Before configuring OAuth in idrflow, you must first set up an OAuth application with an external OAuth 2.0 service provider.
You must register your idrflow server as an OAuth client, and then obtain the required values to complete the configuration in idrflow.
...
| **MCP Scope** | Scope for MCP operations | You define this. As of idrflow 1.6, `user` is the only available value. | `user` |
| **Provider Scope** | OAuth provider scope | You define this. As of idrflow 1.6, `openid` is the only available value. | `openid` |
...
2. Configure the OAuth settings with the values from your OAuth deployment.
    ...
    The OAuth credentials are encrypted and stored securely in your idrflow database.
...
4. If you already installed your idrflow MCP server in your MCP client, you must update your MCP client configuration to use the new OAuth settings after enabling OAuth on your MCP server.
...
Only use this option when idrflow is running in a trusted environment.
...
The following environment variables set behaviors related to your idrflow projects' MCP servers:

| Variable | Format | Default | Description |
|----------|--------|---------|-------------|
| `IDRFLOW_MCP_SERVER_ENABLED` | Boolean | `True` | Whether to initialize an MCP server for each of your idrflow projects. If `false`, idrflow doesn't initialize MCP servers. |
| `IDRFLOW_MCP_SERVER_ENABLE_PROGRESS_NOTIFICATIONS` | Boolean | `False` | If `true`, idrflow MCP servers send progress notifications. |
| `IDRFLOW_MCP_SERVER_TIMEOUT` | Integer | `20` | The number of seconds to wait before an MCP server operation expires due to poor connectivity or long-running requests. |
| `IDRFLOW_MCP_MAX_SESSIONS_PER_SERVER` | Integer | `10` | Maximum number of MCP sessions to keep per unique server. |
| `IDRFLOW_ADD_PROJECTS_TO_MCP_SERVERS` | Boolean | `True` | Whether to automatically add newly created projects to the user's MCP servers configuration. If `false`, projects must be manually added to MCP servers. |

{/* The anchor on this section (deploy-your-server-externally) is currently a link target in the idrflow UI. Do not change. */}
### Deploy your idrflow MCP server externally {#deploy-your-server-externally}

To deploy your idrflow MCP server externally, see [Deploy a public idrflow server](/deployment-public-server).
...
3. In the MCP Inspector UI, enter the connection details for your idrflow project's MCP server.
    ...
    <Tabs groupId="auth-type">
      <TabItem value="API key" label="API key" default>
        ...
        - **Command**: `uvx`
        - **Arguments**: Enter the following list of arguments, separated by spaces. Replace the values for `YOUR_API_KEY`, `IDRFLOW_SERVER_ADDRESS`, and `PROJECT_ID` with the values from your idrflow MCP server. For example:
            ```bash
            mcp-proxy --headers x-api-key YOUR_API_KEY http://IDRFLOW_SERVER_ADDRESS/api/v1/mcp/project/PROJECT_ID/streamable
            ```
      ...
      <TabItem value="None" label="None">
        ...
        - **URL**: Enter the idrflow MCP server's endpoint. For example:
            ```bash
...
## Troubleshoot idrflow MCP servers {#troubleshooting-mcp-server}

For troubleshooting advice for MCP servers and clients, see [Troubleshoot idrflow: MCP issues](/troubleshoot#mcp).

## See also

* [Use idrflow as an MCP client](/mcp-client)
