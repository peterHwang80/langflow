...
To edit idrflow's global model provider configuration, do the following:
...
3. In the **API Key** field, add your provider's API key.
    ...
    You can only add one key for each provider. Make sure the key has access to _all_ models that you want to use in idrflow.
4. Enable the specific models that you want to use in idrflow.
...
After you enable a model in idrflow's global model configuration, you can use that model in any model-driven component in your flows.