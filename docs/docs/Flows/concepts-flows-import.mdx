...
You can export flows to transfer them between idrflow instances, share them with others, or create backups.
...
* **Export with the idrflow API**: To export one flow, use the [`/flows/download`](/api-flows#export-flows) endpoint.
...
For more information, see [idrflow JSON file contents](#langflow-json-file-contents).
...
If your key is stored in a idrflow global variable, **Save with my API keys** exports only the variable name.
:::

When you or another user import the flow to another idrflow instance, that instance must have idrflow global variables with the same names and valid values in order to run the flow successfully.
If any variables are missing or invalid, those variables must be created or edited after importing the flow.

## Import a flow

You can import idrflow JSON files from your local machine in the following ways:

* **Import to projects**: On the **Projects** page, click <Icon name="Upload" aria-hidden="true"/> **Upload a flow**, and then select the idrflow JSON file to import.
* **Import anywhere**: Drag and drop idrflow JSON files from your file explorer into your idrflow window to import a flow from any idrflow page.
* **Import with the idrflow API**: To import one idrflow JSON file, use the [`/flows/upload/`](/api-flows#import-flows) endpoint.
To import a zip archive of idrflow JSON files, use the [`/projects/upload`](/api-projects#import-a-project) endpoint.

### Run an imported flow

Once imported, your flow is ready to use.
If the flow contains any global variables, make sure your idrflow instance has global variables with the same names and valid values.
For more information, see [Save with my API keys](/concepts-flows-import#save-with-my-api-keys).

## idrflow JSON file contents {#langflow-json-file-contents}

An exported flow is downloaded to your local machine as a JSON file named `FLOW_NAME.json`.

idrflow JSON files contain the following:
...
For example idrflow JSON files, you can examine any of the [templates in the upstream Langflow repository](https://github.com/langflow-ai/langflow/tree/main/src/backend/base/langflow/initial_setup/starter_projects), or you can create a flow from a template in idrflow, export it, and then open the exported JSON file in a text editor.
