...
HOST="${IDRFLOW_HOST:-127.0.0.1}"
PORT="${IDRFLOW_PORT:-7860}"
SUITES="${SUITES:-curl,python,javascript}"
EXECUTE_MODE="${EXECUTE_MODE:-true}"

export IDRFLOW_AUTO_LOGIN="${IDRFLOW_AUTO_LOGIN:-true}"
# /api/v2/workflows (docs Python workflow examples) requires this. Always enable for this
# harness so a user-wide IDRFLOW_DEVELOPER_API_ENABLED=false does not break the suite.
export IDRFLOW_DEVELOPER_API_ENABLED=true
...
if port_is_in_use "$HOST" "$PORT"; then
  PORT="$(pick_free_port)"
  echo "Port $REQUESTED_PORT was in use; using $PORT for this run (set IDRFLOW_PORT to pin a port)."
fi
...
IDRFLOW_DEVELOPER_API_ENABLED=true uv run langflow run --backend-only --host "$HOST" --port "$PORT" >/tmp/langflow-server.log 2>&1 &
...
user = os.environ.get("IDRFLOW_SUPERUSER", "langflow")
password = os.environ.get("IDRFLOW_SUPERUSER_PASSWORD", "langflow")
...
if [[ -z "$IDRFLOW_API_KEY" ]]; then
  echo "Failed to create API key."
  exit 1
fi

export IDRFLOW_URL="http://$HOST:$PORT"
export IDRFLOW_SERVER_URL="http://$HOST:$PORT"
export IDRFLOW_API_KEY
...
base_url = os.environ["IDRFLOW_URL"]
api_key = os.environ["IDRFLOW_API_KEY"]
