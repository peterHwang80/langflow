...
| `X-IDRFLOW-GLOBAL-VAR-*` | No | Global variables for the flow | `"X-IDRFLOW-GLOBAL-VAR-API_KEY: sk-..."` For more, see [Pass global variables to your flows in headers](#global-var). |
...
Variables passed with `X-IDRFLOW-GLOBAL-VAR-{VARIABLE_NAME}` are always available to your flow, regardless of whether they exist in the database.
