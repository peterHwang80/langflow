...
idrflow provides several components to help you store and retrieve vector data in your flows, including embedding models, vector stores, and knowledge bases.
...
Knowledge bases are stored local to your idrflow instance.
...
- **idrflow Desktop**:
    ...
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.idrflowDesktop\knowledge_bases`
- **idrflow OSS**:
...
If you set the `IDRFLOW_CONFIG_DIR` environment variable, the `knowledge_bases` subdirectory is created relative to that path.

To change the default `knowledge_bases` directory path, set the `IDRFLOW_KNOWLEDGE_BASES_DIR` environment variable:

```bash
export IDRFLOW_KNOWLEDGE_BASES_DIR="/path/to/parent/directory"
```
