...
Get started with idrflow by loading a template flow, running it, and then serving it at the `/run` API endpoint.
...
- Create a [idrflow API key](/api-keys-and-authentication)
    ...
    <details>
    <summary>Create a idrflow API key</summary>

    A idrflow API key is a user-specific token you can use with idrflow.

    To create a idrflow API key, do the following:

    1. In idrflow, click your user icon, and then select **Settings**.
    2. Click **idrflow API Keys**, and then click <Icon name="Plus" aria-hidden="true"/> **Add New**.
    3. Name your key, and then click **Create API Key**.
    4. Copy the API key and store it securely.
    5. To use your idrflow API key in a request, set a `IDRFLOW_API_KEY` environment variable in your terminal, and then include an `x-api-key` header or query parameter with your request.
    For example:
...
:::tip
If you encounter the error "An API key must be passed as query or header" when attempting to sign up, see [Troubleshooting](/troubleshoot#an-api-key-must-be-passed-as-query-or-header).
:::
...
3. In the **Agent** component, select your configured model from the **Language Model** dropdown.
    ...
    * Edit idrflow's global <Icon name="BrainCog" aria-hidden="true" /> **Models** configuration. These providers and models are part of idrflow's core functionality. Use the **Ollama** provider to connect to any model hosted on a local or remote Ollama instance.
...
idrflow is an IDE, but it's also a runtime you can call through the [idrflow API](/api-reference-api-examples) with Python, JavaScript, or HTTP.

When you start idrflow locally, you can send requests to the local idrflow server.
For production applications, you need to [deploy a stable idrflow instance](/deployment-overview) to handle API calls.

For example, you can use the `/run` endpoint to run a flow and get the result.

idrflow provides code snippets to help you get started with the idrflow API.

1. When editing a flow, click **Share**, and then click **API access**.

    The default code in the API access pane constructs a request with the idrflow server `url`, `headers`, and a `payload` of request data.
    The code snippets automatically include the `IDRFLOW_SERVER_ADDRESS` and `FLOW_ID` values for the flow, and a script to include your `IDRFLOW_API_KEY` if you've set it as an environment variable in your terminal session.
    ...
    url = "http://IDRFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID"  # The complete API endpoint URL for this flow
    ...
    fetch('http://IDRFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID', options)
...
In a production application, you probably want to select parts of this response to return to the user, store in logs, and so on. The next steps demonstrate how you can extract data from a idrflow API response to use in your application.
...
This script runs a question-and-answer chat in your terminal and stores the agent's previous answer so you can compare them.
    ...
    const url = 'http://IDRFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID';
...
To assist with formatting, you can define tweaks in idrflow's **Input Schema** pane before copying the code snippet.
...
idrflow updates the `tweaks` object in the code snippets based on your input parameters, and includes default values to guide you.
...
* [Trigger flows with the idrflow API](/concepts-publish)
* [Use idrflow as a Model Context Protocol (MCP) server](/mcp-server)
* [Containerize a idrflow application](/develop-application)
