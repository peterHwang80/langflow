...
To contribute code or documentation to idrflow, follow the [pull request guide](#open-a-pull-request).
...
1. Fork the idrflow repository on the GitHub host configured by your maintainers.
...
> Note: idrflow tracks the upstream `langflow-ai/langflow` project as an implementation reference. Public contributors should target their team's idrflow fork rather than the upstream repository.
...
* Pull request titles appear in idrflow's release notes, so they should explain what the PR does as explicitly as possible.
* Pull requests should strive to fix one thing **only**, and should contain a good description of what is being fixed.

3. A idrflow maintainer will review your pull request and may request changes, so ensure you pay attention to your PRs. Thanks for your contribution!
