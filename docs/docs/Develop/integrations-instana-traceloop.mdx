...
description: Instrument idrflow with the Traceloop SDK, and export traces and metrics to Instana using OpenTelemetry.
---
...
This guide demonstrates how to integrate the Instana observability platform with your idrflow application using the Traceloop SDK so you can monitor and analyze LLM performance.
...
1. In the root folder of your idrflow application, edit your existing idrflow `.env` file or create a new one.
...
## Start idrflow with Traceloop environment variables

Launch your idrflow application with your `.env` file:
...
1. Run a flow in idrflow to generate traffic.
2. To view traces in Instana, open Instana and click **Applications**.
3. In **Services**, search for `idrflow`.
4. Click **idrflow** to view and analyze the associated calls.
