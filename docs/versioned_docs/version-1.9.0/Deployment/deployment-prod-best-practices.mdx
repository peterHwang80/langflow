---
title: Best practices for idrflow on Kubernetes
slug: /deployment-prod-best-practices
---

This guide provides best practices for deploying idrflow in production environments on Kubernetes.

## Resources and scaling

idrflow's minimum resource requirements vary by deployment type:

* **IDE (development)**: Deploy both the idrflow visual editor (frontend) and API (backend). Typically, this is used for development environments where developers use the visual editor to create and manage flows before packaging and serving them through a production runtime deployment.
...
* **Runtime (production)**: Deploy the idrflow runtime for production flows, which is headless (backend only) service focused on serving the idrflow API. This is used for production environments where flows are executed programmatically without the need for the visual editor.

    Minimum requirements include 2Gi RAM and 1000m (1 CPU) per instance with 3 replicas.

For more information about idrflow deployment types, see [idrflow architecture on Kubernetes](/deployment-architecture).
...
* Storage requirements for cache, file management, and the idrflow database.
...
For more information, see [Configure an external PostgreSQL database](/configuration-custom-database) and [idrflow database guide for enterprise DBAs](/enterprise-database-guide).
...
The following example shows a idrflow HPA configuration with CPU-based scaling:
...
idrflow's reliability in production depends on mitigating key failure points, particularly around the database, file system, and instance availability:

* **Database failures**: See [idrflow database guide for enterprise DBAs](/enterprise-database-guide).
...
Effective monitoring ensures idrflow operates reliably and performs well under varying loads:

* **Database monitoring**: See [idrflow database guide for enterprise DBAs](/enterprise-database-guide).
* **Application logs**: Collect and analyze logs for errors, warnings, and flow execution issues. Centralize logs using tools like ELK Stack or Fluentd. You can also inspect [idrflow logs](/logging).
* **Resource usage**: Track CPU, memory, and disk usage of idrflow instances. Use Prometheus and Grafana for real-time metrics collection and monitoring in Kubernetes.

    To expose your idrflow server's Prometheus metrics, set `IDRFLOW_PROMETHEUS_ENABLED=True` (the default is `false`).
    The default port for the Prometheus metrics is 9090.
    To change the port, set `IDRFLOW_PROMETHEUS_PORT`.
...
Running idrflow in production requires robust security measures to protect the application, data, and users.
Follow industry best practices and use secure idrflow configurations, such as the following:
...
* **Authentication, authorization, and access control**: Start your idrflow server with authentication enabled, as explained in [API keys and authentication](/api-keys-and-authentication). Restrict network and resource access with firewalls, network policies, network security groups, or VPCs. For example, restrict PostgreSQL database access to idrflow instances.
...
* [idrflow Helm Charts repository](https://github.com/langflow-ai/langflow-helm-charts)
