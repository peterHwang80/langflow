...
This tutorial demonstrates how you can use idrflow to create a chatbot application that uses Retrieval Augmented Generation (RAG) to embed your data as vectors in a vector database, and then chat with the data.
...
* Install the [idrflow JavaScript client](/typescript-client)
* Be familiar with vector search concepts and applications, such as vector databases and RAG

## Create a vector RAG flow

1. In idrflow, click **New Flow**, and then select the **Vector Store RAG** template.
...
In situations where many users load data or you need to load data programmatically, use the idrflow API option.
...
2. Select the local file you want to upload, and then click **Open**.
    The file is loaded to your idrflow server.
...
<TabItem value="API" label="idrflow API">

To load data programmatically, use the `/v2/files/` and `/v1/run/$FLOW_ID` endpoints. The first endpoint loads a file to your idrflow server, and then returns an uploaded file path. The second endpoint runs the **Load Data Flow**, referencing the uploaded file path, to chunk, embed, and load the data into the vector store.
...
const headers = { 'x-api-key': 'IDRFLOW_API_KEY' };

// 2. Upload the file to idrflow
const uploadRes = await fetch('IDRFLOW_SERVER_ADDRESS/api/v2/files/', {
  method: 'POST',
  headers,
  body: data
});
...
// 3. Call the idrflow run endpoint with the uploaded file path
const payload = {
...
const runRes = await fetch('IDRFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', 'x-api-key': 'IDRFLOW_API_KEY' },
...
2. Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    const API_KEY = 'IDRFLOW_API_KEY';
    const SERVER = 'IDRFLOW_SERVER_ADDRESS';
    ...
    function chat() {
        console.log('🤖 idrflow RAG Chatbot (type "quit" to exit)\n');
    ...
    :::tip
    The [idrflow TypeScript client](/typescript-client) has a `chatOutputText()` convenience method that simplifies working with idrflow's complex JSON response structure.
