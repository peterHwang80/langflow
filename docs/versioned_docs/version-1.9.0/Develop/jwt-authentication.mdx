...
No configuration is necessary, but if you want to explicitly set it in the idrflow `.env`, the default value is `LANGFLOW_ALGORITHM=HS256`.
...
To automatically generate a private and public key pair and store it in the idrflow [`LANGFLOW_CONFIG_DIR`](/logging), set `LANGFLOW_ALGORITHM="RS256"` in your idrflow `.env`.
...
To automatically generate a private and public key pair and store it in the idrflow [`LANGFLOW_CONFIG_DIR`](/logging), set `LANGFLOW_ALGORITHM="RS512"` in your idrflow `.env`.
