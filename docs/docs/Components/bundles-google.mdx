...
idrflow integrates with [Google BigQuery](https://cloud.google.com/bigquery) through the **BigQuery** component in the [**Google** bundle](/bundles-google), allowing you to execute SQL queries and retrieve data from your BigQuery datasets.
...
* A [running idrflow server](/get-started-installation)
...
This role allows idrflow to access BigQuery resources with the service account.
...
Now that you have a service account and a JSON private key, you need to configure the credentials in the idrflow **BigQuery** component.

#### Configure credentials in the idrflow component

With your service account configured and your credentials JSON file created, follow these steps to authenticate the idrflow application.

1. Create a new flow in idrflow.
...
idrflow includes multiple components that support Google Search, such as the following:
...
The **Google OAuth Token** component was deprecated in idrflow 1.4.0.
