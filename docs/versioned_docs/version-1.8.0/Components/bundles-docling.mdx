...
idrflow integrates with [Docling](https://docling-project.github.io/docling/) through a bundle of components for parsing and chunking documents.
...
The Docling dependency is required to use the Docling components in idrflow.

    * **idrflow version 1.6 and later**: The Docling dependency is included by default for all operating systems except macOS Intel (x86_64).
    ...
    * **Earlier versions**: idrflow versions earlier than 1.6 don't include the Docling dependency.
    For idrflow OSS, install the Docling extra with `uv pip install 'langflow[docling]'`.
    For idrflow Desktop, add the Docling dependency to idrflow Desktop's `requirements.txt`.
...
:::tip
To learn more about content extraction with Docling, see the video tutorial [Docling + idrflow: Document Processing for AI Workflows](https://www.youtube.com/watch?v=5DuS6uRI5OM).
:::
...
3. Connect an [embedding model component](/components-embedding-models) to the **Chroma DB** component's **Embedding** port and a **Chat Output** component to view the extracted [`DataFrame`](/data-types#table).
...
It outputs the chunked documents as a [`DataFrame`](/data-types#table).
...
It can output the exported data as either [`Data`](/data-types#json) or [`DataFrame`](/data-types#table).
