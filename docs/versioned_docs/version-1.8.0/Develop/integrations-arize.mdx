...
To enable Arize tracing, set the required Arize environment variables in your idrflow deployment.
Arize begins monitoring and collecting telemetry data from your LLM applications automatically.

:::tip
Instructions for integrating idrflow and Arize are also available in the Arize documentation:

* [idrflow tracing with Arize Platform](https://arize.com/docs/ax/integrations/frameworks-and-platforms/langflow/langflow-tracing)
* [idrflow tracing with Arize Phoenix](https://arize.com/docs/phoenix/integrations/langflow/langflow-tracing)
:::
...
2. In the root of your idrflow application, edit your existing idrflow `.env` file or create a new one.
...
4. Start your idrflow application with your `.env` file:
...
1. In idrflow, run a flow that has an LLM-driven component, such as an **Agent** component or any language model component.
...
3. To view metrics for your flows, go to the **LLM Tracing** tab.
    ...
    Each idrflow execution generates two traces in Arize:

    * The `AgentExecutor` trace is the Arize trace of LangChain's `AgentExecutor`.
    * The `UUID` trace is the trace of the idrflow components.
