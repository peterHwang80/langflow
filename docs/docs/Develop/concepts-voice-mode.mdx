...
:::info
Voice mode is not available in idrflow Desktop.
To use voice mode, [Install the idrflow OSS Python package](/get-started-installation#install-and-run-the-langflow-oss-python-package).
:::

You can use idrflow's voice mode to interact with your flows verbally through a microphone and speakers.
...
* An [OpenAI](https://platform.openai.com/) account and an OpenAI API key because idrflow uses the OpenAI API to process voice input and generate responses.
...
5. Enter your OpenAI API key, and then click **Save**. idrflow saves the key as a [global variable](/configuration-global-variables).
...
idrflow saves this key as a global variable.
...
idrflow exposes two OpenAI Realtime API-compatible websocket endpoints for your flows.
...
The idrflow API's websockets endpoints require an [OpenAI API key](https://platform.openai.com/docs/overview) for authentication, and they support an optional [ElevenLabs](https://elevenlabs.io) integration with an ElevenLabs API key.
...
This is the mode used in the idrflow **Playground**.
...
If omitted, idrflow uses the flow ID as the [session ID](/session-id).
