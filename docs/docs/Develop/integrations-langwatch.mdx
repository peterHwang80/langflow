...
To integrate with idrflow, add your LangWatch API key as a idrflow environment variable:

1. Get a LangWatch API key from your LangWatch account.

2. Add the key to your idrflow `.env` file:
...
3. Restart idrflow with your `.env` file, if you modified the idrflow `.env`:
