---
title: idrflow TypeScript client
...
The idrflow TypeScript client allows your TypeScript applications to programmatically interact with the idrflow API.
...
## Install the idrflow TypeScript package

To install the idrflow typescript client package, use one of the following commands:
...
## Initialize the idrflow TypeScript client
...
2. Initialize a `idrflowClient` object to interact with your server:
    ...
    The default idrflow base URL is `http://localhost:7860`.
    To create an API key, see [API keys and authentication](/api-keys-and-authentication).

## Connect to your server and get responses

1. With your idrflow client initialized, test the connection by calling your idrflow server.
    ...
    * `baseUrl`: The URL of your idrflow server.
...
2. Review the result to confirm that the client connected to your idrflow server.

    The following example shows the response from a well-formed `runFlow` request that reached the idrflow server and successfully started the flow:
...
This example builds on the quickstart with additional features for interacting with idrflow:
...
3. Instead of calling `run` on the Flow object, call `stream` with the same arguments to get a streaming response:
    ...
    For more information on streaming idrflow responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
## Retrieve idrflow logs with the TypeScript client

To retrieve [idrflow logs](/logging), you must enable log retrieval on your idrflow server by including the following values in your idrflow `.env` file:

```text
IDRFLOW_ENABLE_LOG_RETRIEVAL=True
IDRFLOW_LOG_RETRIEVER_BUFFER_SIZE=10000
IDRFLOW_LOG_LEVEL=DEBUG
```
