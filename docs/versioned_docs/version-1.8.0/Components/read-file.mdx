...
| file_path | Server File Path | Input parameter. A `Data` object with a `file_path` property pointing to a file in [idrflow file management](/concepts-file-management) or a `Message` object with a path to the file. Supersedes **Files** (`path`) but supports the same file types. |
...
- **Structured Content**: Available only for `.csv`, `.xlsx`, `.parquet`, and `.json` files.
    ...
    - For `.csv` files, produces a [`DataFrame`](/data-types#table) representing the table data.
    - For `.json` files, produces a [`Data`](/data-types#json) object with the parsed JSON data.
...
- **Structured Output**: A [`DataFrame`](/data-types#table) containing the Docling-processed document data with text elements, page numbers, and metadata.
...
If you upload multiple files, the component outputs **Files**, which is a [`DataFrame`](/data-types#table) containing the content and metadata of all selected files.
