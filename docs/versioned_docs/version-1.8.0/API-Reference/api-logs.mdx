...
Retrieve logs for your idrflow flows and server.

## Enable log retrieval

The `/logs` endpoint requires log retrieval to be enabled in your idrflow instance.

To enable log retrieval, set the following [environment variables](/environment-variables) in your idrflow `.env` file, and then start idrflow with `uv run langflow run --env-file .env`:

```text
IDRFLOW_ENABLE_LOG_RETRIEVAL=True
IDRFLOW_LOG_RETRIEVER_BUFFER_SIZE=10000 // Must be greater than 0
IDRFLOW_LOG_LEVEL=DEBUG // Can be DEBUG, ERROR, INFO, WARNING, or CRITICAL
```
