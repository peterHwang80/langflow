...
:::info
The `/build` endpoints are used by idrflow's frontend visual editor code.
These endpoints are part of the internal idrflow codebase.

Don't use these endpoints to run flows in applications that use your idrflow flows.
To run flows in your apps, see [Flow trigger endpoints](/api-flows-run).
:::

The `/build` endpoints support idrflow's frontend code for building flows in the idrflow visual editor.
You can use these endpoints to build vertices and flows, as well as execute flows with streaming event responses.
You might need to use or understand these endpoints when contributing to the idrflow codebase.
...
| start_component_id | string | Optional. ID of the component where the execution should start. Component `id` values can be found in [idrflow JSON files](/concepts-flows-import#langflow-json-file-contents) |
| stop_component_id | string | Optional. ID of the component where the execution should stop. Component `id` values can be found in [idrflow JSON files](/concepts-flows-import#langflow-json-file-contents).|
...
The `/build` endpoint also accepts inputs for `data` directly, instead of using the values stored in the idrflow database.
