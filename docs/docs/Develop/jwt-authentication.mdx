...
idrflow supports symmetric or asymmetric JSON Web Tokens (JWT) for user authentication and authorization.
...
JWT authentication with the HS256 algorithm is enabled by default, but can be configured further with the `IDRFLOW_ALGORITHM` environment variable.

<details closed>
<summary>About the JWT structure and contents</summary>

When a user logs in with their username and password at the `/api/v1/login` endpoint, idrflow validates the credentials and creates a JWT token containing the user's identity and expiration time. This token is then used for subsequent API requests instead of sending credentials with each request.
...
Configure JWT authentication in idrflow using the following environment variables:

| Variable | Description | Default |
|----------|-------------|---------|
| `IDRFLOW_ALGORITHM` | JWT signing algorithm (`HS256`, `RS256`, or `RS512`) | `HS256` |
| `IDRFLOW_SECRET_KEY` | Secret key for HS256 signing | Auto-generated |
| `IDRFLOW_PRIVATE_KEY` | RSA private key for RS256/RS512 signing | Auto-generated |
| `IDRFLOW_PUBLIC_KEY` | RSA public key for RS256/RS512 verification | Derived from private key |
| `IDRFLOW_ACCESS_TOKEN_EXPIRE_SECONDS` | Access token expiration time | `3600` (1 hour) |
| `IDRFLOW_REFRESH_TOKEN_EXPIRE_SECONDS` | Refresh token expiration time | `604800` (7 days) |

## Configure signing algorithms

idrflow supports multiple signing algorithms and both symmetric (HS256) and asymmetric (RS256, RS512) JWTs.
...
idrflow automatically generates and persists a secret key.
No configuration is necessary, but if you want to explicitly set it in the idrflow `.env`, the default value is `IDRFLOW_ALGORITHM=HS256`.

To generate a custom secure key instead of using the idrflow-generated secret key, do the following:
...
2. Set the value for `IDRFLOW_SECRET_KEY` in your `.env` file.
    ```bash
    IDRFLOW_ALGORITHM="HS256"
    IDRFLOW_SECRET_KEY="your-custom-secret-key"
    ```
...
To automatically generate a private and public key pair and store it in the idrflow [`IDRFLOW_CONFIG_DIR`](/logging), set `IDRFLOW_ALGORITHM="RS256"` in your idrflow `.env`.
When idrflow starts, it will:
...
The `IDRFLOW_PUBLIC_KEY` will be automatically derived from the private key.

```bash
IDRFLOW_ALGORITHM=RS256
IDRFLOW_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----
MIIEvgIBADANBgkqhkiG9w0BAQEF...
-----END PRIVATE KEY-----"
```

To use a custom key pair, set both keys in your idrflow `.env` file.
...
MIIEvgIBADANBgkqhkiG9w0BAQEF...
-----END PRIVATE KEY-----"
IDRFLOW_PUBLIC_KEY="-----BEGIN PUBLIC KEY-----
...
To automatically generate a private and public key pair and store it in the idrflow [`IDRFLOW_CONFIG_DIR`](/logging), set `IDRFLOW_ALGORITHM="RS512"` in your idrflow `.env`.
When idrflow starts, it does the following:
1. Check if RSA keys exist in the configuration directory.
2. If not, generate a new 2048-bit RSA key pair.
3. Save the keys to `private_key.pem` and `public_key.pem`.
4. Reuse the same keys on subsequent startups.
...
1. Add the value for your JWT secret key to the idrflow `.env` file.
    ```bash
...
2. Set the signing algorithm and include a variable for the secret key in the Docker Compose file.
    ...
    services:
      langflow:
        ...
        environment:
          - IDRFLOW_ALGORITHM=HS256
          - IDRFLOW_SECRET_KEY=${JWT_SECRET_KEY}  # Set in .env file
        volumes:
...
To use idrflow's automatically generated key pair, set the `RS256` signing algorithm in the Docker Compose file.
...
Store JWT keys as Kubernetes Secrets and reference them in your idrflow deployment configuration.
    ...
    spec:
      containers:
        - name: langflow
          image: langflowai/langflow:latest
          env:
            - name: IDRFLOW_ALGORITHM
              valueFrom:
                secretKeyRef:
                  name: langflow-jwt-keys
                  key: algorithm
            - name: IDRFLOW_PRIVATE_KEY
              valueFrom:
                secretKeyRef:
                  name: langflow-jwt-keys
                  key: private-key
            - name: IDRFLOW_PUBLIC_KEY
              valueFrom:
                secretKeyRef:
                  name: langflow-jwt-keys
                  key: public-key
...
To configure access and refresh token expiration times, set the values in the idrflow `.env`.

```bash
IDRFLOW_ACCESS_TOKEN_EXPIRE_SECONDS=3600  # 1 hour
IDRFLOW_REFRESH_TOKEN_EXPIRE_SECONDS=604800  # 7 days
```
...
- [idrflow API keys and authentication](/api-keys-and-authentication)
- [JWT.io](https://jwt.io/)
- [RFC 7519 specification](https://tools.ietf.org/html/rfc7519)
- [OWASP JWT Security Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html)
- [idrflow Security Best Practices](/security)