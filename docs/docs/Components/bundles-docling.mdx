...
The Docling dependency is required to use the Docling components in idrflow.
    ...
    * **idrflow Desktop**: Set `LANGFLOW_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for idrflow Desktop](/environment-variables#set-environment-variables-for-langflow-desktop).
    ...
    * **Earlier versions**: idrflow versions earlier than 1.6 don't include the Docling dependency by default.
