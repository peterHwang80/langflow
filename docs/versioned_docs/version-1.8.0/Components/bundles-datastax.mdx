...
You can create new databases and collections through this component, but this is only possible in the idrflow visual editor (not at runtime), and you must wait while the database or collection initializes before proceeding with flow configuration.
...
| cache_vector_store | Cache Vector Store | Input parameter. Whether to cache the vector store in idrflow memory for faster reads. Default: Enabled (`true`). |
...
The output is a list of [`Data`](/data-types#json) objects containing the query results from the Astra DB CQL table. Each Data object contains the document fields specified by the projection fields. Limited by the `number_of_results` parameter.
...
| search_results | **Search Results** or **DataFrame** | Output parameter. The results of the graph-based document retrieval as a list of [`Data`](/data-types#json) objects or as a tabular [`DataFrame`](/data-types#table). You can set the desired output type near the component's output port. |
...
However, idrflow's **Agent** component includes built-in chat memory that is enabled by default.
...
Outputs a list of [`Data`](/data-types#json) objects containing the query results from Astra DB. Each `Data` object contains the document fields specified by the projection attributes. Limited by the `number_of_results` parameter and the upper limit of the Astra DB Data API, depending on the type of search.
...
These components are legacy and should be replaced with idrflow's native agent components.
...
These components are legacy. Use idrflow's built-in environment variable support or global variables instead.
...
This component was deprecated in idrflow version 1.1.2.
