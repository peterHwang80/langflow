...
This page summarizes significant changes to idrflow in each release.
For the full upstream change log, review the upstream Langflow release history.

Due to strict SemVer requirements, idrflow Desktop can have different patch versions than the core idrflow OSS Python package, but the major and minor versions are aligned.

## Prepare to upgrade

:::warning
Whenever possible, the idrflow team recommends installing new idrflow versions in a new virtual environment or VM before upgrading your primary installation.
...
To avoid the impact of potential breaking changes and test new versions, the idrflow team recommends the following upgrade process:

1. Recommended: [Export your projects](/api-projects#export-a-project) to create backups of your flows:

    ```bash
    curl -X GET \
    "$IDRFLOW_SERVER_URL/api/v1/projects/download/$PROJECT_ID" \
...
2. Install the new version:
   ...
   * **idrflow OSS Python package**: Install the new version in a new virtual environment. For instructions, see [Install and run the idrflow OSS Python package](/get-started-installation#install-and-run-the-langflow-oss-python-package).
   * **idrflow Docker image**: Run the new image in a separate container.
   * **idrflow Desktop**: To upgrade in place, open idrflow Desktop, and then click **Upgrade Available** in the idrflow header. If you want to isolate the new version, you must install idrflow Desktop on a separate physical or virtual machine, and then [import your flows](/concepts-flows-import) to the new installation.
...
- idrflow MCP client for Bob (IBM) and Claude Code

    The **Settings** page now includes <Icon name="Terminal" aria-hidden="true"/> **idrflow MCP Client**, with instructions to connect **Bob (IBM)** and **Claude Code** to your idrflow instance.
    For more information, see [idrflow MCP client for coding agents](../Agents/idrflow-mcp-client.mdx).
...
- MCP Tools: register servers first, and then add from the MCP sidebar
    ...
    **MCP Tools** is no longer added from the **Agents** component list. Register each MCP server under **Settings** > **MCP Servers**, or from the flow editor <McpIcon /> **MCP** sidebar.
    ...
    For more information, see [Use idrflow as an MCP client](/mcp-client#use-the-mcp-tools-component).

- Flow DevOps Toolkit SDK

    Use the Flow DevOps Toolkit SDK to manage local and remote flows with Git-style terminal commands.
    For more information, see the [idrflow Flow DevOps Toolkit SDK](../API-Reference/flow-devops-sdk.mdx).
...
- Renamed `Data` and `DataFrame` types
    ...
    For more information, see [idrflow data types](/data-types).
...
- Build components with **idrflow Assistant**

    **idrflow Assistant** is an in-app virtual assistant that can create components from natural language prompts directly within the idrflow workspace.
    For more information, see [idrflow Assistant](../Flows/idrflow-assistant.mdx).
...
- Vulnerability reporting now uses HackerOne
    ...
    For the full policy and what to include in a report, see the [idrflow Security Policy](http://218.50.209.93:9001/peter/idrflow/-/blob/main/SECURITY.md).

- Disable custom components

    Set `IDRFLOW_ALLOW_CUSTOM_COMPONENTS=false` to disable custom components and in-editor editing of component code.
...
- Deploy flows to watsonx Orchestrate (beta)
    ...
    In idrflow 1.9.1, idrflow flows can now be published to IBM watsonx Orchestrate as tools that a watsonx Orchestrate agent can call.
    This workflow packages a selected flow version for use in IBM watsonx Orchestrate.
    For more information, see [Deploy idrflow on watsonx Orchestrate](../Deployment/deployment-wxo.mdx).
...
-   `langflow-base` dependency structure refactored
    ...
    If you installed idrflow with `uv pip install langflow`, this isn't a breaking change. Installing `langflow` automatically installs `langflow-base[complete]`, which includes all optional dependencies and maintains the same functionality as before.

    However, if you installed idrflow with `uv pip install langflow-base` without specifying extra dependencies, this _is_ a breaking change.
    Some dependencies that were previously included by default are now available only through optional extras.
    Therefore, installing `langflow-base` directly only installs the [core base dependencies](http://218.50.209.93:9001/peter/idrflow/-/blob/main/src/backend/base/pyproject.toml).
...
-   Docker image does not include `uv` or `uvx`
    ...
    :::tip
    Starting with idrflow 1.8.1, the official Docker images include `uv` and `uvx` again.
    If you're using idrflow 1.8.0, follow the steps in this release note to add `uv` and `uvx` in a derived image.
    :::

    In idrflow 1.8.0, the idrflow Docker image does not include `uv` or `uvx` in the runtime image.
    This means that MCP server configurations, including the default idrflow MCP server, that rely on commands like `uvx mcp-proxy` will fail inside the container with a `command not found` error.

    If you use MCP from within a idrflow 1.8.0 Docker image, you must install `uv` in an image derived from the official `langflowai/langflow` image.

    To install `uv` and `uvx` in a derived image based on the official `langflowai/langflow` image, do the following:

    1. Create a `Dockerfile` in your project.
    2. Use the official idrflow image as the base and install `uv` into the existing virtual environment:
        ```dockerfile
    ...
    For more information, see [Upgrade the idrflow Docker image](/deployment-docker#upgrade-the-langflow-docker-image).
...
- Docling installation behavior differs between Desktop and OSS
    ...
    In idrflow 1.8.0, Docling behavior depends on how idrflow is installed.

    For idrflow Desktop installations, you need to set `IDRFLOW_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for idrflow Desktop](/environment-variables#set-environment-variables-for-langflow-desktop).

    For idrflow OSS installations with `uv pip install langflow`, Docling is included with the idrflow package.

    For idrflow OSS installations with `uv pip install langflow-base`, install Docling explicitly as an extra with `uv pip install "langflow-base[docling]"`.
...
- Developer API: `/workflow` synchronous endpoints (Beta)
    ...
    The Developer API is part of a larger effort to improve idrflow's APIs with enhanced capabilities and better developer experience.
...
- Traces and trace view
    ...
    idrflow now records execution traces for flows and components.
...
- More configuration options for JWT-based session authentication
    ...
    idrflow 1.8 offers additional configuration options for JWT algorithms, including support for RS256/RS512 algorithms, configurable keys, and token lifetimes. For more information, see [JWT authentication](/jwt-authentication).
...
- Docker AMD vs ARM image sizes
    ...
    idrflow 1.8.0 addresses the AMD vs ARM Docker image size gap.
...
- New [**Openlayer** observability integration](/integrations-openlayer)
    ...
    Configures idrflow to send tracing data to Openlayer for analysis, monitoring, and evaluation of your flow executions.
...
All idrflow versions upgrading to 1.7.0 are affected.

Don't upgrade to idrflow version 1.7.0.
...
1. Revert idrflow to version 1.6.9:
...
- Support for streamable HTTP transport for MCP clients and servers
    ...
    idrflow now supports [streamable HTTP transport](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) for both MCP clients and servers.
    When using idrflow as an [MCP client](/mcp-client), you can connect to MCP servers with streamable HTTP transport.
    When using idrflow as an [MCP server](/mcp-server), clients can connect using streamable HTTP transport.
    SSE transport is still supported as a fallback for backwards compatibility.

- Webhook authentication

    Added the `IDRFLOW_WEBHOOK_AUTH_ENABLE` environment variable for authenticating requests to the [`/webhook` endpoint](/api-flows-run#webhook-run-flow). When `IDRFLOW_WEBHOOK_AUTH_ENABLE=TRUE`, webhook endpoints require API key authentication and validate that the authenticated user owns the flow being executed. When `FALSE`, no idrflow API key is required and all requests to the webhook endpoint are treated as being sent by the flow owner. For more information, see [Trigger flows with webhooks](/webhook).

- Configurable API key validation

    Added the `IDRFLOW_API_KEY_SOURCE` environment variable to control how idrflow validates API keys. When set to `db`, idrflow validates API keys against keys stored in the database. When set to `env`, idrflow validates API keys against the `IDRFLOW_API_KEY` environment variable. For more information, see [API keys and authentication](/api-keys-and-authentication#langflow-api-key-source).

- SSRF protection

    Added SSRF (Server-Side Request Forgery) protection to the [**API Request** component](/api-request). HTTP redirects are disabled by default to prevent SSRF bypass attacks. To enable SSRF protection, set `IDRFLOW_SSRF_PROTECTION_ENABLED=TRUE`. Configure allowed hosts with `IDRFLOW_SSRF_ALLOWED_HOSTS`. Flows that relied on automatic redirects will need to enable it manually.

- Email registration in idrflow Desktop

    idrflow Desktop now includes an email registration screen at startup. The registered email address helps track user registrations and understand the user base. For more information, see [Telemetry](/contributing-telemetry#email).
...
idrflow versions 1.6.0 through 1.6.3 have a critical bug where environment variables from `.env` files aren't read.
...
If your `.env` file includes `AUTO_LOGIN=false`, upgrading to the impacted versions causes idrflow to fall back to default settings, potentially giving all users superuser access immediately upon upgrade.
Additionally, database credentials, API keys, and other sensitive configurations can't be loaded from `.env` files.

_Don't_ upgrade to any idrflow version from 1.6.0 through 1.6.3 if you use `.env` files for configuration.
...
:::warning
Windows users of idrflow Desktop should **not** use the in-app update feature to upgrade to idrflow version 1.6.0.
Only Windows Desktop users upgrading to idrflow version 1.6.0 are affected.
:::

The **Update** button in idrflow Desktop will not work for Windows users, and may result in data loss.

Instead, install a fresh build from your approved idrflow distribution channel when a new version is available.

Follow the instructions below to minimize the risk of losing flows.

These instructions assume the default path for the idrflow database of `C:\Users\YOUR_USERNAME\AppData\Roaming\com.langflow\data\database.db` or `C:\Users\%YOUR_USERNAME%\AppData\Roaming\com.langflow\data\database.db`. Replace `YOUR_USERNAME` with your Windows username.

1. Ensure you have Administrator privileges.
2. Ensure you have enough disk space for a second installation of idrflow.
3. Close idrflow, and ensure no idrflow process is running in Task Manager.
4. The idrflow 1.6.0 installer automatically performs a database backup in a later step, but an additional manual backup provides additional redundancy and is recommended.
To manually back up your idrflow database file, do the following:
...
5. Download the Windows installer from your approved idrflow distribution channel.
...
The idrflow 1.6.0 installer automatically performs a database backup.
These steps install idrflow from scratch, and result in two idrflow installations: the previously installed version, and version 1.6.0.
This is expected behavior.
8. Start version 1.6.0 of idrflow, and confirm your flows behave as expected.
9. **If flows are missing**, restore your flows from your manual backup by doing the following:
    ...
    4. Start idrflow, and confirm your flows behave as expected.
    5. After confirmation, uninstall the previous version of idrflow, and keep version 1.6.0.

### Breaking changes

- Authentication enforced for idrflow API requests by default

    In idrflow version 1.6, `IDRFLOW_AUTO_LOGIN=True` and `IDRFLOW_SKIP_AUTH_AUTO_LOGIN=False` by default.
    This enforces authentication for idrflow API requests while still automatically authenticating all users as superusers in the visual editor.
    ...
    **This is a breaking change from 1.5 where both of these environment variables were `true` by default, bypassing all authentication.**
    ...
    For more information, see the documentation for [`IDRFLOW_AUTO_LOGIN`](/api-keys-and-authentication#langflow-auto-login).

### New features and enhancements

- OpenAI Responses API compatibility

  idrflow now includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses) at `POST /api/v1/responses`.
...
- Advanced document parsing with built-in Docling support
  ...
  To make it easier to use the [**Docling** components](/bundles-docling) and the **Read File** component's new advanced parsing feature, the Docling dependency is now included with idrflow for all operating systems except macOS Intel (x86_64).
...
- idrflow API requests can require authentication

    To enhance security and ensure proper authentication for automatic login features, idrflow API endpoints now require authentication with a idrflow API key, even when `IDRFLOW_AUTO_LOGIN=True`.
    This change will be enforced in a future release.
    For temporary backwards compatibility, this release adds the `IDRFLOW_SKIP_AUTH_AUTO_LOGIN` environment variable.
    The default value is `true`, which disables API authentication enforcement.
    To enforce API authentication, set `IDRFLOW_SKIP_AUTH_AUTO_LOGIN=False`.
    For more information, see the documentation for [`IDRFLOW_AUTO_LOGIN`](/api-keys-and-authentication#langflow-auto-login).
...
- MCP server one-click installation
    ...
    On your idrflow project's **MCP server** page, click **Auto install** to install your idrflow MCP server to MCP clients with just one click.
    The option to install with a JSON configuration file is available for macOS, Windows, and WSL.
    For more information, see [Use idrflow as an MCP server](/mcp-server).

- MCP server management

    You can now add, remove, and edit your MCP servers in the **MCP Tools** components and through your idrflow **Settings** page.
    For more information, see [Use idrflow as an MCP client](/mcp-client).
...
- Stability improvements
    ...
    See an issue? Track it in the repository workflow used by your idrflow fork.
...
- Added an enhanced **Breaking Changes** feature to help update components without breaking flows after updating idrflow.
...
- Introduced MCP server functionality to serve idrflow tools to MCP-compatible clients.
...
See the upstream Langflow release history for the full implementation changelog.