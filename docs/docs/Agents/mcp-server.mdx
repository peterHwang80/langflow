...
|----------|--------|---------|-------------|
| `LANGFLOW_MCP_SERVER_ENABLED` | Boolean | `True` | Whether to initialize an MCP server for each of your idrflow projects. If `false`, idrflow doesn't initialize MCP servers. |
| `LANGFLOW_MCP_SERVER_ENABLE_PROGRESS_NOTIFICATIONS` | Boolean | `False` | If `true`, idrflow MCP servers send progress notifications. |
...
3. In the MCP Inspector UI, enter the connection details for your idrflow project's MCP server.
    ...
    <Tabs groupId="auth-type">
      <TabItem value="API key" label="API key" default>
        ...
        - **Command**: `uvx`
        - **Arguments**: Enter the following list of arguments, separated by spaces. Replace the values for `YOUR_API_KEY`, `LANGFLOW_SERVER_ADDRESS`, and `PROJECT_ID` with the values from your idrflow MCP server. For example:
            ```bash
