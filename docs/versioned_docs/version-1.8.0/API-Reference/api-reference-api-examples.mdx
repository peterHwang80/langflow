...
export IDRFLOW_SERVER_URL="https://localhost:7860"
...
```bash
curl -X GET \
  "$IDRFLOW_SERVER_URL/health_check" \
...
```bash
curl -X GET \
  "$IDRFLOW_SERVER_URL/api/v1/version" \
  -H "accept: application/json"
```
...
```bash
curl -X GET \
  "$IDRFLOW_SERVER_URL/api/v1/config" \
  -H "accept: application/json" \
