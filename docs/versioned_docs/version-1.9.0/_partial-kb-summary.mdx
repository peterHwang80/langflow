A idrflow knowledge base is a local vector database that is stored in idrflow storage.
...
* **Local storage**: idrflow knowledge bases are exclusively local.
In contrast, only some vector store components support local databases.
* **Built-in embedding models**: idrflow knowledge bases include built-in support for several embedding models.
...
* **Basic similarity search**: When querying idrflow knowledge bases, only standard similarity search is supported.
...
* **Structured data**: idrflow knowledge bases only support structured data.
