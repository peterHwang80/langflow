...
* **Config directory**: Set `LANGFLOW_SAVE_DB_IN_CONFIG_DIR=True` to store the database in your idrflow config directory as set in [`LANGFLOW_CONFIG_DIR`](/logging).
...
* **No database**: To disable all database operations and run a no-op session, set `LANGFLOW_USE_NOOP_DATABASE=True` in your [idrflow environment variables](/environment-variables).
...
• **Flow**: Contains flow definitions, including nodes, edges, and components, stored as JSON or database records. For more information, see [Build flows](/concepts-flows).
    ...
    :::tip
    To automatically remove API keys and tokens from flow data before saving a flow to the database, set `LANGFLOW_REMOVE_API_KEYS=True` in your [idrflow environment variables](/environment-variables).
...
To replace the default idrflow SQLite database with another database, set the `LANGFLOW_DATABASE_URL` environment variable to your database URL, and then start idrflow with your `.env` file.
...
* `LANGFLOW_DATABASE_CONNECTION_RETRY`: Whether to retry lost connections to your idrflow database. If `true`, idrflow tries to connect to the database again if the connection fails. Default: `false`.
...
* `LANGFLOW_MIGRATION_LOCK_NAMESPACE`: Optional namespace identifier for PostgreSQL advisory lock during migrations. If not provided, a hash of the database URL will be used. Useful when multiple idrflow instances share the same database and need coordinated migration locking
...
|----------|------|---------|-------------|
| `LANGFLOW_CACHE_TYPE` | String | `async` | Set the cache type for idrflow's internal caching system. Possible values: `async`, `redis`, `memory`, `disk`. If you set the type to `redis`, then you must also set the `LANGFLOW_REDIS_*` environment variables. |
| `LANGFLOW_LANGCHAIN_CACHE` | String | `InMemoryCache` | Set the cache storage type for the LangChain caching system (a idrflow dependency), either `InMemoryCache` or `SQLiteCache`. |
