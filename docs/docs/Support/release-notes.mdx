...
- Docling installation behavior differs between Desktop and OSS
    ...
    For idrflow Desktop installations, you need to set `LANGFLOW_DOCLING=True` in your `.env` file to enable Docling dependency installation. For more information, see [Set environment variables for idrflow Desktop](/environment-variables#set-environment-variables-for-langflow-desktop).
...
- Authentication enforced for idrflow API requests by default
    ...
    In idrflow version 1.6, `LANGFLOW_AUTO_LOGIN=True` and `LANGFLOW_SKIP_AUTH_AUTO_LOGIN=False` by default.
...
- idrflow API requests can require authentication
    ...
    To enhance security and ensure proper authentication for automatic login features, idrflow API endpoints now require authentication with a idrflow API key, even when `LANGFLOW_AUTO_LOGIN=True`.
