...
In the idrflow **Playground**, current sessions are listed on the left side of the pane.

idrflow uses session IDs to track different chat interactions within flows. This allows multiple chat sessions to exist in a single flow. Messages are stored in the database with session IDs as a reference.
...
Replace `IDRFLOW_SERVER_ADDRESS`, `FLOW_ID`, and `IDRFLOW_API_KEY` with the values from your idrflow deployment.
```
curl --request POST \
...
To retrieve messages from local idrflow memory, add a [**Message History** component](/message-history) to your flow.
...
Messages can be retrieved by `session_id` from the idrflow API at `GET /v1/monitor/messages`. For more information, see [Monitor endpoints](https://docs.langflow.org/api-monitor).

For an example of session ID in action, see [Use Session IDs in idrflow](https://www.youtube.com/watch?v=nJiF_eF21MY).