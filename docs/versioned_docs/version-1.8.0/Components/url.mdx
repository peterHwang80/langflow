...
* **Output data type**: In the component's output field (near the output port) you can select the structure of the outgoing data when it is passed to other components:
    ...
    * **Extracted Pages**: Outputs a [`DataFrame`](/data-types#table) that breaks the crawled pages into columns for the entire page content (`text`) and metadata like `url` and `title`.
