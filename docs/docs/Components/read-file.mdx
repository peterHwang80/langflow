...
In idrflow version 1.7.0, this component was renamed from **File** to **Read File**.

The **Read File** component loads and parses files, converts the content into a `JSON`, `Table`, or `Message` object.
...
The **Read File** component can read files from the local idrflow database, **AWS S3**, or **Google Drive**.
...
If you need to load an unsupported file type, you must use a different component that supports that file type and, potentially, parses it outside idrflow, or you must convert it to a supported type before uploading it.
...
|------|--------------|------|
| path | Files | Input parameter. The path to files to load. Can be local or in [idrflow file management](/concepts-file-management). Supports individual files and bundled archives. |
| file_path | Server File Path | Input parameter. A `JSON` object with a `file_path` property pointing to a file in [idrflow file management](/concepts-file-management) or a `Message` object with a path to the file. Supersedes **Files** (`path`) but supports the same file types. |
...
| ignore_unspecified_files | Ignore Unspecified Files | Input parameter. If `true`, `JSON` with no `file_path` property is ignored. If `false` (default), the component errors when a file isn't specified. |
...
Starting in idrflow version 1.6, the **Read File** component supports advanced document parsing using the [Docling](https://docling-project.github.io/docling/) library for supported file types.

To use advanced parsing, do the following:

1. Complete the following prerequisites, if applicable:

    * **Install idrflow version 1.6 or later**: Earlier versions don't support advanced parsing with the **Read File** component. For upgrade guidance, see the [Release notes](/release-notes).
    ...
    * **Enable Developer Mode for Windows**:
    <PartialDevModeWindows />
        ...
        Developer Mode isn't required for idrflow OSS on Windows.
...
- **Structured Content**: Available only for `.csv`, `.xlsx`, `.parquet`, and `.json` files.
    ...
    - For `.csv` files, produces a [`Table`](/data-types#table) representing the table data.
    - For `.json` files, produces a [`JSON`](/data-types#json) object with the parsed JSON data.

- **Raw Content**: A [`Message`](/data-types#message) containing the file's raw text content.

- **File Path**: A [`Message`](/data-types#message) containing the path to the file in [idrflow file management](/concepts-file-management).
...
- **Structured Output**: A [`Table`](/data-types#table) containing the Docling-processed document data with text elements, page numbers, and metadata.
...
If you upload multiple files, the component outputs **Files**, which is a [`Table`](/data-types#table) containing the content and metadata of all selected files.
