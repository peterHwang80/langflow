...
Follow these best practices when submitting a template to idrflow.

For template formatting examples, see [`/starter_projects`](https://github.com/langflow-ai/langflow/tree/main/src/backend/base/langflow/initial_setup/starter_projects) in the upstream Langflow repository.

## Create an MR to submit your template
...
2. On your GitLab clone or fork, add your `template.json` file to `/starter_projects`.
3. Include the [required items for template submission](#required-items-for-template-submission).
4. Create a merge request from your branch to the canonical idrflow repository or release branch defined by your team.
5. Include a screenshot of your template in the MR.

The idrflow team will review your MR, offer feedback, and, if approved, merge the template.
...
Don't use custom components that aren't part of the idrflow codebase.
...
For more information, see the upstream Langflow repository's [template categories](https://github.com/langflow-ai/langflow/blob/main/src/frontend/src/modals/templatesModal/index.tsx#L27-L57).
