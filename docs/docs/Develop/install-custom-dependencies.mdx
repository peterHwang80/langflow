...
idrflow provides optional dependency groups and support for custom dependencies to extend idrflow functionality. This guide covers how to add dependencies for different idrflow installations, including idrflow Desktop and idrflow OSS.

The idrflow codebase uses three packages, each with its own `pyproject.toml` file:
...
* The `lfx` package is managed at `src/lfx/pyproject.toml`. LFX is a lightweight CLI tool for executing and serving idrflow flows. The `lfx` package does not provide optional dependency groups for end users.

## Install custom dependencies in idrflow Desktop {#langflow-desktop}

To add dependencies to idrflow Desktop, add an entry for the package to the application's `requirements.txt` file:

    * On macOS, the file is located at `/Users/USER/.langflow/data/requirements.txt`.
    * On Windows, the file is located at `C:\Users\USER\AppData\Roaming\com.idrflow\data\requirements.txt`.

Add each dependency to `requirements.txt` on its own line in the format `DEPENDENCY==VERSION`, such as `matplotlib==3.10.0`.

Restart idrflow Desktop to install the dependencies.

If you need to change or uninstall custom dependencies, edit the `requirements.txt` file, and then restart idrflow Desktop.

## Install custom dependencies in idrflow OSS

To install your own custom dependencies in your idrflow environment, add them with your package manager.

If you're working within a cloned idrflow repository, add dependencies with `uv add` because there is already a `pyproject.toml` file for uv to reference:
...
These optional dependencies are listed in the [langflow `pyproject.toml`](http://218.50.209.93:9001/peter/idrflow/-/blob/main/pyproject.toml) file under `[project.optional-dependencies]`.
...
`langflow-base` is recommended when you want to deploy idrflow with specific dependencies only.
...
These optional dependency groups are listed in the [langflow-base `pyproject.toml`](http://218.50.209.93:9001/peter/idrflow/-/blob/main/src/backend/base/pyproject.toml) file under `[project.optional-dependencies]`.
...
For example, if you want to experiment with a custom dependency like `matplotlib` with idrflow:

```bash
# Create and activate a virtual environment
uv venv YOUR_IDRFLOW_VENV
source YOUR_IDRFLOW_VENV/bin/activate
...
If you're working within a cloned idrflow repository, add dependencies with `uv add` to reference the existing `pyproject.toml` files:
...
## Add dependencies to the idrflow codebase

When contributing to the idrflow codebase, you might need to add dependencies to idrflow.
