---
title: Get started with the idrflow API
slug: /api-reference-api-examples
---
import CodeSnippet from '@site/src/components/CodeSnippet';
import exampleApiReferenceApiExamplesFormidrflowApiRequests from '!!raw-loader!@site/docs/API-Reference/curl-examples/api-reference-api-examples/form-langflow-api-requests.sh';
...
import examplePythonApiReferenceApiExamplesFormidrflowApiRequests from '!!raw-loader!@site/docs/API-Reference/python-examples/api-reference-api-examples/form-langflow-api-requests.py';
import exampleJavascriptApiReferenceApiExamplesFormidrflowApiRequests from '!!raw-loader!@site/docs/API-Reference/javascript-examples/api-reference-api-examples/form-langflow-api-requests.js';
...
You can use the idrflow API for programmatic interactions with idrflow, such as the following:

* Create and edit flows, including file management for flows.
* Develop applications that use your flows.
* Develop custom components.
* Build idrflow as a dependency of a larger application, codebase, or service.
* Contribute to the overall idrflow codebase.

To view and test all available endpoints, you can access the idrflow API's OpenAPI specification at your idrflow deployment's `/docs` endpoint, such as `http://localhost:7860/docs`.

:::tip Try it
For an example of the idrflow API in a script, see the [idrflow quickstart](/get-started-quickstart).
...
## Form idrflow API requests

While individual options vary by endpoint, all idrflow API requests share some commonalities, like a URL, method, parameters, and authentication.

As an example of a idrflow API request, the following curl command calls the `/v1/run` endpoint, and it passes a runtime override (`tweaks`) to the flow's **Chat Output** component:

<Tabs>
<TabItem value="Python" label="Python" default>

<CodeSnippet source={examplePythonApiReferenceApiExamplesFormidrflowApiRequests} language="python" />

</TabItem>
<TabItem value="JavaScript" label="JavaScript">

<CodeSnippet source={exampleJavascriptApiReferenceApiExamplesFormidrflowApiRequests} language="javascript" />

</TabItem>
<TabItem value="curl" label="curl">

<CodeSnippet source={exampleApiReferenceApiExamplesFormidrflowApiRequests} language="bash" />
...
By default, local deployments serve the idrflow API at `http://localhost:7860/api`.

Remotely hosted idrflow deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:IDRFLOW_PORT/api`.

You can configure the idrflow port number in the `IDRFLOW_PORT` [environment variable](/environment-variables).

* `https://UUID.ngrok.app/api`
* `http://IP_OR_DNS/api`
* `http://IP_OR_DNS:IDRFLOW_PORT/api`

### Authentication

In idrflow versions 1.5 and later, most API endpoints require authentication with a idrflow API key in either an `x-api-key` header or query parameter.
...
idrflow API requests use various methods, paths, path parameters, query parameters, and body parameters.
...
The idrflow API serves `/v1` and `/v2` endpoints.
...
Commonly used values in idrflow API requests include your [idrflow server URL](#base-url), [idrflow API keys](#authentication), flow IDs, and [project IDs](/api-projects#read-projects).
...
## Try some idrflow API requests

Once you have your idrflow server URL, try calling these endpoints that return idrflow metadata.

### Health check

Returns the health status of the idrflow database and chat services:
...
idrflow provides an additional `GET /health` endpoint.
This endpoint is served by uvicorn before idrflow is fully initialized, so it's not reliable for checking idrflow service health.

### Get version

Returns the current idrflow API version:
...
Returns configuration details for your idrflow deployment.
Requires a [idrflow API key](/api-keys-and-authentication).
...
Returns a dictionary of all idrflow components.
Requires a [idrflow API key](/api-keys-and-authentication).
...
Because you can run idrflow as either an IDE (frontend and backend) or a runtime (headless, backend-only), it serves endpoints that support frontend and backend operations.
Many endpoints are for orchestration between the frontend and backend, reading and writing to the idrflow database, or enabling frontend functionality, like the **Playground**.
Unless you are contributing to the idrflow codebase, you won't directly call most of the idrflow endpoints.
...
The following endpoints are useful for developing applications with idrflow and administering idrflow deployments with one or more users.
...
* Deployment details:
  * GET `/v1/version`: Return idrflow version. See [Get version](/api-reference-api-examples#get-version).
...
* Custom components: You might use these endpoints when developing custom idrflow components for your own use or to share with the idrflow community:
  * GET `/v1/all`: Return all available idrflow component types. See [Get all components](/api-reference-api-examples#get-all-components).
...
The following endpoints are most often used when contributing to the idrflow codebase, and you need to understand or call endpoints that support frontend-to-backend orchestration or other internal functionality.
...
* MCP servers: The following endpoints are for managing idrflow MCP servers and MCP server connections.
They aren't typically called directly; instead, they are used to drive internal functionality in the idrflow frontend and when running flows that call MCP servers.
idrflow MCP servers support both streamable HTTP and SSE transport.
...
* Use the idrflow API to [run a flow](/api-flows-run).
* Use the idrflow API to [upload files](/api-files).
* Use the idrflow API to [get flow logs](/api-logs).
* Explore all endpoints in the [idrflow API specification](/api).
