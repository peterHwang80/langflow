...
idrflow logs are stored in the config directory specified in the `LANGFLOW_CONFIG_DIR` environment variable.
...
|----------|--------|---------|-------------|
| `LANGFLOW_CONFIG_DIR` | String | Varies | Set the idrflow configuration directory where files and logs are stored. Default path depends on your installation, as described in the preceding list. |
...
| `LANGFLOW_ENABLE_LOG_RETRIEVAL` | Boolean | `False` | Enables retrieval of logs from your idrflow instance with [Logs endpoints](/api-logs). |
...
| `LANGFLOW_NATIVE_TRACING` | Boolean | `true` | Enables the tracer to record execution traces directly in the idrflow database for use in Trace View. Set to `false` to disable tracing. |
