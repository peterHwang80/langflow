...
- idrflow MCP client for Bob (IBM) and Claude Code
    ...
    For more information, see [idrflow MCP client for coding agents](../Agents/langflow-mcp-client.mdx).
...
- Build components with **idrflow Assistant**
    ...
    **idrflow Assistant** is an in-app virtual assistant that can create components from natural language prompts directly within the idrflow workspace.
    For more information, see [idrflow Assistant](../Flows/langflow-assistant.mdx).
...
Instead, download a fresh installation from the [idrflow website](https://langflow.org) when a new version is available.
...
5. Download the Windows installer from the [idrflow website](https://langflow.org).
