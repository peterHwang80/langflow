...
For example, if you have `LANGFLOW_PORT=7860` defined as an environment variable, and you run the CLI with `--port 7880`, then idrflow sets the port to `7880` because the CLI option overrides the environment variable.
...
To disable the `langflow superuser` command, you must set `LANGFLOW_ENABLE_SUPERUSER_CLI=False` in your idrflow `.env` file, and then [start idrflow with your `.env` file](#start-langflow-with-a-specific-env-file).