...
idrflow flows are fully serializable and can be saved and loaded from the file system where idrflow is installed.

:::tip
To try building and running a flow in a few minutes, see the [idrflow quickstart](/get-started-quickstart).
:::
...
* **Create a flow from a template**: Select a project, click **New Flow**, and then click the template that you want to use.
    ...
    You can also [contribute templates](/contributing-templates) to the idrflow codebase.
...
You can also create a flow with the [idrflow API](/api-flows), but the idrflow team recommends using the [visual editor](/concepts-overview) until you are familiar with flow creation.
...
When you're ready to use idrflow for application development, learn how to [trigger flows with the idrflow API](/concepts-publish), explore more advanced configuration options like [custom dependencies](/install-custom-dependencies), and, eventually, [containerize your idrflow application](/develop-application).

When you're ready to go to production or deploy a idrflow MCP server for access over the public internet, see [idrflow deployment overview](/deployment-overview).

#### Flow graphs

When a flow runs, idrflow builds a Directed Acyclic Graph (DAG) object from the nodes (components) and edges (connections), and the nodes are sorted to determine the order of execution.
...
The **Projects** page is where you arrive when you launch idrflow.
From here, you can manage flows and your projects' [MCP servers](/mcp-server).

idrflow projects are like folders that you can use to organize related flows.
...
:::tip
To get back to the **Projects** page after editing a flow, click the project name or idrflow icon in the idrflow header.
:::
...
Saved flow versions are stored in the database configured for your idrflow deployment.

When restoring a saved version, if **Save current draft before restoring** is enabled, then idrflow creates a backup of your current work before the selected version replaces it.
...
When previewing a saved version, idrflow temporarily pauses auto-save so the preview doesn't overwrite your current draft.
...
By default, flows and flow execution data are stored in the idrflow database, and flow logs are stored with other idrflow logs in the idrflow config directory.
