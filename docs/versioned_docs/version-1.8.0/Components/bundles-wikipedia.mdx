...
It accepts a string text query, and it outputs a [`DataFrame`](/data-types#table) containing the search results.

## Wikipedia API

This component searches and retrieves information from Wikipedia with the [WikiMedia API](https://www.mediawiki.org/wiki/API:Main_page) search endpoints, and then outputs the response as a [`DataFrame`](/data-types#table).
