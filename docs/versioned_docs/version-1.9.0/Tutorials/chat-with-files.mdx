...
The main focus of this tutorial is to show you how to provide files as input to a idrflow flow, so your chatbot can use the content of those files in its responses.
...
1. In idrflow, click **New Flow**, and then select the **Basic Prompting** template.
...
To do this, your application must send a `POST /run` request to your idrflow server with the file you want to upload and a text prompt.
The result includes the outcome of the flow run and the LLM's response.

This example uses a local idrflow instance, and it asks the LLM to evaluate a sample resume.
...
1. To construct the request, gather the following information:
    ...
    * `FILE_COMPONENT_ID`: The UUID of the **Read File** component in your flow, such as `File-KZP68`. To find the component ID, open your flow in idrflow, click the **Read File** component, and then click **Controls**. The component ID is at the top of the **Controls** pane.
...
2. Copy the following script into a Python file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    url = "http://IDRFLOW_SERVER_ADDRESS/api/v2/files/"
    ...
    # 5. Call the idrflow run endpoint with the uploaded file path
    run_url = "http://IDRFLOW_SERVER_ADDRESS/api/v1/run/FLOW_ID"
    run_payload = {
    ...
    The first request uploads a file, such as `fake-resume.txt`, to your idrflow server at the `/v2/files` endpoint. This request returns a file path that can be referenced in subsequent idrflow requests, such as `02791d46-812f-4988-ab1c-7c430214f8d5/fake-resume.txt`

    The second request sends a chat message to the idrflow flow at the `/v1/run/` endpoint.
...
3. Save and run the script to send the requests and test the flow.
    ...
    The initial output contains the JSON response object from the file upload endpoint, including the internal path where idrflow stores the file.
...
def chat_with_flow(input_message, file_paths):
    ...
    run_url = f"{IDRFLOW_SERVER_ADDRESS}/api/v1/run/{FLOW_ID}"
...
To upload files from another machine that isn't your local environment, your idrflow server must first be accessible over the internet. Then, authenticated users can upload files to your public idrflow server's `/v2/files/` endpoint, as shown in the tutorial. For more information, see [idrflow deployment overview](/deployment-overview).
...
In the visual editor, you can preload files to the **Read File** component by selecting them from your local machine or [idrflow file management](/concepts-file-management).
