...
Instead of manually exporting, sharing, and importing flow JSON files from the idrflow UI, the Flow DevOps toolkit offers terminal-based workflows for versioning, environment variables, testing, and deployment.

## Prerequisites

- [Install and start idrflow](/get-started-installation)
- Create a [idrflow API key](/api-keys-and-authentication)
- Install the idrflow `lfx` package
   ...
   2. Activate the virtual environment.
      ...
      ```
   3. Install the idrflow LFX package in the virtual environment:
      ```bash
      uv pip install lfx
      ```
   4. Run Flow DevOps Toolkit commands in the virtual environment that has LFX installed.

      Alternatively, you can run `uvx lfx` commands, or run LFX from the `src/lfx` directory in a cloned idrflow repo.
      For more information, see the [upstream Langflow LFX README](https://github.com/langflow-ai/langflow/blob/main/src/lfx/README.md).
...
3. To initialize a project, run:
    ...
    ✔ Project scaffolded. Next steps:
      ...
      2. export IDRFLOW_LOCAL_API_KEY=<key>   (Settings -> API Keys)
...
4. Add your idrflow API key to your `.env` file, or export it within the terminal session.
    ...
    For example, to add a idrflow API key to a local idrflow server, set:

    ```text
    export IDRFLOW_LOCAL_API_KEY=IDRFLOW_API_KEY
    ```
...
7. In the idrflow UI, open the Simple Agent flow and change the flow.
...
9. To pull the reported changes from the idrflow server to your local project folder, run `lfx pull`.
10. To _push_ flow changes from flows stored locally in `demo-project/flows` to the idrflow server, run `lfx push`.

## Validate flows

The Flow DevOps SDK can validate that local flows are correctly formed before pushing to the idrflow with `lfx validate`.
...
2. Optionally, you can now share and serve the flow by keeping the flow JSON and `requirements.txt` in the same environment.
   To serve the flow without the idrflow UI, do the following:
   ...
   4. To set a idrflow API key, run:
   ```
   export IDRFLOW_API_KEY=IDRFLOW_API_KEY
   ```

   5. To serve the flow without the idrflow UI, pass the flow JSON path to the `lfx serve` command:
   ```
...
production:
  ...
  api_key_env: IDRFLOW_PRODUCTION_API_KEY
```

Each entry contains a `url` for the idrflow base URL, and an `api_key_env` field.
...
`environments.yaml` is distinct from the idrflow or LFX `.env` file.
`environments.yaml` controls which remote idrflow instance you're deploying flows to, flow versioning, and environment variable _names_ for API keys.
The `.env` contains runtime values for the idrflow server, and might also contain _actual secret values_, so the `.env` should not be committed to version control.

Commands that call a idrflow server over HTTP, such as `lfx pull` or `lfx push`, use `--env ENVIRONMENT_NAME` to determine which idrflow instance to send the request to.
...
This command will send the request to the idrflow base URL at `http://127.0.0.1:7860` using a idrflow API key named `IDRFLOW_LOCAL_API_KEY`.
