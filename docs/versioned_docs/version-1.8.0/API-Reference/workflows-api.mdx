...
The Workflow API provides programmatic access to execute idrflow workflows synchronously or asynchronously.
...
The Workflow API is part of the idrflow Developer v2 API and offers enhanced workflow execution capabilities compared to the v1 `/run` endpoint.
...
url = f"{IDRFLOW_SERVER_URL}/api/v2/workflows"
headers = {
...
const url = `${IDRFLOW_SERVER_URL}/api/v2/workflows`;
...
url = f"{IDRFLOW_SERVER_URL}/api/v2/workflows/stop"
headers = {
    "Content-Type": "application/json",
    "x-api-key": IDRFLOW_API_KEY
}
...
const url = `${IDRFLOW_SERVER_URL}/api/v2/workflows/stop`;
...
To find the component ID in the idrflow UI, open your flow in idrflow, click the component, and then click **Controls**. The component ID is at the top of the **Controls** pane.
