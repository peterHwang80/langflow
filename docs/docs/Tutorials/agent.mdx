...
This tutorial shows you how to connect a JavaScript application to a [idrflow agent](/agents).
...
In this example, the application sends a customer's email address to the idrflow agent. The agent compares the customer's previous orders within the **Directory** component, searches the web for used versions of those items, and returns three results.
...
If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
    ...
    const IDRFLOW_SERVER_ADDRESS = 'IDRFLOW_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';
    const IDRFLOW_API_KEY = 'IDRFLOW_API_KEY';
    ...
    async function runAgentFlow(): Promise<void> {
        try {
            ...
            const client = new LangflowClient({
                baseUrl: IDRFLOW_SERVER_ADDRESS,
                apiKey: IDRFLOW_API_KEY
            });

            console.log(`Connecting to idrflow server at: ${IDRFLOW_SERVER_ADDRESS} `);
            ...
            if (error instanceof Error) {
                if (error.message.includes('fetch')) {
                    console.error('\nMake sure your idrflow server is running and accessible at:', IDRFLOW_SERVER_ADDRESS);
                }
    ...
    // Run the function
    console.log('Starting idrflow Agent...\n');
...
3.  Save and run the script to send the request and test the flow.
    ...
    ```
    Starting idrflow Agent...

    Connecting to idrflow server at: http://localhost:7860
