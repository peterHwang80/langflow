---
title: Build components with idrflow Assistant
slug: /idrflow-assistant
---

import Icon from "@site/src/components/icon";

**idrflow Assistant** is an in-app virtual assistant pane within the **Playground**.

It can answer questions about the application and help you get more out of idrflow.

**idrflow Assistant** understands the structure of the idrflow graph, so it can create components from natural language prompts.

Behind the scenes, **idrflow Assistant** runs a built-in idrflow flow on your idrflow server each time you send a message.
This flow is distinct from the flow that is open in the canvas, and has its own language model.
The language model in **idrflow Assistant** only has the currently opened flow in your workspace for context.
To give **idrflow Assistant** context for a different flow, switch to that flow in your workspace, and open **idrflow Assistant**.

## Prerequisites

* Connect an LLM provider in the [Global model providers page](/components-models) for **idrflow Assistant** to use

## Create a custom component with idrflow Assistant

In this example, you'll prompt **idrflow Assistant** to create a custom component that validates and normalizes a list of URLs.

You'll then iterate on the code based on the results in the **Playground**.

1. In the **Playground**, click the <Icon name="idrflow" aria-hidden="true"/> **idrflow** icon.
    The **idrflow Assistant** pane opens.
2. Optionally, ask `What can you help me with?` for a list of **idrflow Assistant's** capabilities.
3. Prompt **idrflow Assistant** to generate a custom component. For example:
    ```
...
4. **idrflow Assistant** generates component code from your prompt. Because generation is model-driven, your code may differ from the example below.
    <details closed>
    ...
    class URLTitleExtractor(Component):
        ...
        def _process_url(self, url: str, timeout: float) -> dict[str, Any]:
            ...
            request = Request(
                url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; idrflow URLTitleExtractor/1.0)"
                },
            )
...
6. Open the **Playground**, and tell idrflow to check a list of URLs.
    For example:
    ```
    Check these links: https://example.com
...
7. Run the flow.
    ...
    | http://218.50.209.93:9001/peter/idrflow | GitLab - peter/idrflow: idrflow is a powerful tool for building and deploying AI-powered agents and workflows. - GitLab | 200 |
    | https://python.org | Welcome to Python.org | 200 |
    | https://this-domain-should-not-resolve-12345.invalid |  |  |

8. To iterate further, tell **idrflow Assistant** what you want.
    For example, prompt it to `Update URLTitleExtractor to add max_urls (default 5) and skip duplicates.`.

    **idrflow Assistant** generates an updated **URLTitleExtractor** component (again, the exact code may differ from a previous run).
...
10. Run the flow.
    The output will be similar to the following, with the duplicate `https://example.com` removed and only 3 valid URLs displayed.
