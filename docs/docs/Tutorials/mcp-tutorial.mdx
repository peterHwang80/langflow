...
1. For this tutorial's Python script, gather the following information:
    ...
    * `LANGFLOW_SERVER_ADDRESS`: Your idrflow server's domain. The default value is `127.0.0.1:7860`. You can get this value from the code snippets on your flow's [**API access** pane](/concepts-publish#api-access).
    ...
    * `LANGFLOW_API_KEY`: A valid [idrflow API key](/api-keys-and-authentication).
