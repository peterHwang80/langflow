...
idrflow includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses).
...
To be compatible with idrflow's OpenAI Responses API endpoint, your flow and request must adhere to the following requirements:
...
idrflow uses `x-api-key` headers for authentication, while OpenAI uses `Authorization: Bearer` headers.
When sending requests to idrflow with OpenAI client libraries, you must configure custom headers and include an `api_key` configuration.
...
In the following examples, replace the values for `IDRFLOW_SERVER_URL`, `IDRFLOW_API_KEY`, and `FLOW_ID` with values from your deployment.
<Tabs groupId="client">
...
client = OpenAI(
  base_url="IDRFLOW_SERVER_URL/api/v1/",
  default_headers={"x-api-key": "IDRFLOW_API_KEY"},
...
|--------|----------|-------------|---------|
| `x-api-key` | Yes | Your idrflow API key for authentication | `"sk-..."` |
...
The response contains fields that idrflow sets dynamically and fields that use OpenAI-compatible defaults.
...
Fields set dynamically by idrflow:
...
```bash
curl -X POST \
  "http://$IDRFLOW_SERVER_URL/api/v1/responses" \
  -H "x-api-key: $IDRFLOW_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "$FLOW_ID",
...
This example demonstrates passing an `OPENAI_API_KEY` variable, which is a variable idrflow automatically detects from environment variables, with two custom variables for `USER_ID` and `ENVIRONMENT`. The variables don't have to be created in idrflow's Global Variables section - you can pass any variable name in the `X-LANGFLOW-GLOBAL-VAR-{VARIABLE_NAME}` header format.
...
If your flow components reference variables that aren't provided in headers or your idrflow database, the flow fails by default.
