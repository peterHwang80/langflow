...
The **Batch Run** component runs a language model over _each row of one text column_ in a [`DataFrame`](/data-types#table), and then returns a new `DataFrame` with the original text and an LLM response.
