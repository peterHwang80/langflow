...
If you're working within a cloned upstream Langflow repository, add dependencies with `uv add` because there is already a `pyproject.toml` file for uv to reference:
...
If you're working within a cloned upstream Langflow repository, add dependencies with `uv add` to reference the existing `pyproject.toml` files:
