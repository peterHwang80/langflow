---
title: Deploy idrflow with Nginx and SSL
slug: /deployment-nginx-ssl
---

import Icon from "@site/src/components/icon";

Deploy idrflow on a Linux-based server using Nginx as a reverse proxy, Let's Encrypt for SSL certificates, and Certbot for automated certificate management.

This setup encrypts all communications between users and your idrflow server. SSL certificates ensure that sensitive data is protected from eavesdropping and tampering, and the automatic certificate management through Certbot eliminates the complexity of manual SSL configuration.
...
## Install idrflow on your server

To install idrflow on your server, do the following:
...
3. Install uv to manage Python packages.
    idrflow recommends uv for faster installation.
    ```bash
    pip install uv
    ```

4. Create a virtual environment for idrflow.
    ```bash
    uv venv langflow-venv
    source langflow-venv/bin/activate
    ```

5. Install idrflow using uv.
    ```bash
    uv pip install langflow
    ```

6. Optionally, start idrflow.
    ```bash
    uv run langflow run --host 127.0.0.1 --port 7860 &
    ```

    Test access at your `http://YOUR_PUBLIC_IP:7860`.
    Since idrflow is running on `localhost`, you should not be successful.
...
Nginx is a reverse proxy that receives external requests and forwards them to your idrflow server. In addition to SSL termination, Nginx includes features for load balancing and security. For more information, see the [Nginx documentation](https://nginx.org/en/docs/).
...
3. Start idrflow from the virtual environment.
    ...
    b. Start idrflow in the background.
    ```bash
    uv run langflow run --host 127.0.0.1 --port 7860 &
    ```
...
- [Deploy idrflow on a remote server with Caddy](/deployment-caddyfile)
