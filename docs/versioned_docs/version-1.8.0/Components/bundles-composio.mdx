...
You can provide this key directly in your components, or you can use the `COMPOSIO_API_KEY` [global variable](/configuration-global-variables), which idrflow can automatically load from your `.env` file.
...
In non-agentic use cases, the output is a [`DataFrame`](/data-types#table) containing the response from the specified Composio service, depending on the component and action used in the flow.
